...
## function: Rextinct::criwm by Salvador Herrando-Perez (salherra@gmail.com)
...
## object 2 of 2: out$criwm = extinction or arrival time (2.5%, 50% and 97.5% quartiles)
...
#' @name criwm
#'
#' @title Calibrated Re-sampled Inverse-Weighted McInerney (CRIWM)
#'
#' @description Estimate extinction time using a time series of dated observations using the true probability distribution resulting from radiocarbon calibration.
#'
#' @details The fossil record is highly fragmented such that the youngest fossil of an extinct species is unlikely to belong to the last individual of that species (so-called Signor-Lipps effect by Raup 1986). Among the methods available for estimating extinction time (see Wang and Marshall 2016), CRIWM controls for the Signor-Lipps effect based on the frequency of dated fossils prior to the extinction event.
#' CRIWM can also estimate arrival time by conceptualizing that the oldest fossil of a species recorded in a region is unlikely to belong to the first individual colonizing that region.
#' Unlike GRIWM (Bradshaw et al 2012), CRIWM has been designed to account for radiocarbon dating uncertainty by means of a re-sampling procedure using calibrated probability density functions (Keenan 2012).
#'
#' Like GRIWM, CRIWM defines extinction time as the calendar year at which the probability \code{alpha} of finding a fossil younger than the youngest known fossil is low (alpha = 0.05).
#' Analogously, CRIWM defines arrival time as the calendar year at which the probability \code{alpha} of finding a fossil older than the oldest known fossil is low (alpha = 0.05).
...
#' When input data consists of a time series of dates in calendar years (\code{calibra = FALSE}), re-sampling is implemented from a normal distribution, and CRIWM and GRIWM computations and results are identical.
#'
#' \code{criwm} can compute biased and unbiased estimates of extinction (or arrival) time.
...
#'
#' \code{criwm} accepts any combination of chronological data (e.g., radiocarbon, ESR, OSL, TL, Uranium series). Re-sampling is implemented on the true probability density function of radiocarbon dates using \code{\link[base]{sample}}, and on a normal distribution of non-radiocarbon dates using \code{\link[stats]{rnorm}}.
...
#'
#' If \code{calibra = FALSE}, all dates assumed to be in calendar years and re-sampled from a normal distribution, so CRIWM functions exactly like GRIWM.
#'
...
#' \code{cal_save = TRUE} saves the original data with dates in years (non-calibrated) and calendar years (calibrated). Non-calibrated dates only included if \code{calibra = TRUE}. Name of output file = outputCRIWM_calibration.
#' \code{resample_save = TRUE} saves re-sampled extinction (or arrival) values used to estimate median extinction (or arrival) time with 95 percent quartile ranges. Number of values set by \code{resample}. Name of output file = outputCRIWM_resampled_Extinction (\code{signor_lipps = "ext"}) or outputCRIWM_resampled_Arrival (\code{signor_lipps = "arr"}).
#' \code{criwm_save = TRUE} saves median extinction (or arrival) time with 95 percent quartile ranges. Name of output file = outputCRIWM_medianCI_Extinction (\code{signor_lipps = "ext"}) or outputCRIWM_medianCI_Arrival (\code{signor_lipps = "arr"}).
#'
...
#'
#' @param signor_lipps Character. CRIWM accounts for the Signor-Lipps effect at the youngest (extinction) or oldest (arrival) end of a time series of dated observations.
...
#' Default is the original CRIWM estimator \code{biased = TRUE} dependent on parameter \code{alpha}.
...
#' Default \code{alpha = 0.05}. Lacks functionality for CRIWM unbiased estimator (\code{biased = FALSE}). See details.
#'
...
#'
#' @param criwm_save Logical. Determines if median extinction (or arrival) time and 95 percent CI are saved as text file to working directory.
#' Default \code{criwm_save = "FALSE"}.
#'
#' @return Returns a list containing two elements:
#' \itemize{
#' \item {\code{criwm_arguments}} {Data.frame with 2 columns.
#'  Column 1 = arguments in function \code{criwm}, Column 2 = values selected by user.}\cr
...
#'  \item {\code{criwm}} {Data.frame with 3 columns and 3 rows.
...
#'
#' Keenan, D.J. (2012) Calibration of a radiocarbon age. Nonlinear Processes in Geophysics, 19, 345-350.
#'
...
#'
#' @export
utils::globalVariables(c("i_date", "i_criwm", "i_iter", "i_resample")) #assign non-binding global variables (used as index in loops)
criwm <- function(chrono_data, #input data
                  ...
                  cal_save = FALSE, resample_save = FALSE, criwm_save = FALSE) #saving parameters
{
  ...
  if (signor_lipps == "arr") {message(paste("########CRIWM-arrival analysis started / Time = ", date(), "\n"))}
  ...
  if (signor_lipps == "arr") {message("CRIWM arrival time reported in calendar years BP, BCE and BCY\n")}
  ...
  ##############################################
  ## SAVE CRIWM MEDIAN EXTINCTION (OR ARRIVAL) TIME AND 95% CI
  ...
  if (criwm_save == TRUE & signor_lipps == "arr") {
    data.table::fwrite(x = out, file = "outputCRIWM_medianCI_Arrival", col.names = TRUE)
    message("Median arrival time and 95% confidence interval saved to working directory (criwm_save = TRUE) with file name = outputCRIWM_medianCI_Arrival\n")
  }
  ...
  if (signor_lipps == "arr" & isTRUE(resample_save)) {
    data.table::fwrite(x = data.frame(w.T.mci.vec), file = "outputCRIWM_resampled_Arrival", col.names = FALSE)
    message("Re-sampled arrival times saved to working directory (resample_save = TRUE) with file name = outputCRIWM_resampled_Arrival\n")
  }
  ...
  if (signor_lipps == "arr") {message(paste("########CRIWM-arrival analysis ended / Time = ", date(), "\n"))}
